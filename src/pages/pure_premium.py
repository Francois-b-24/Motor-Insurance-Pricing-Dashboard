...
Frequency x Severity decomposition, pure premium analysis by segment,
severity distribution, model vs actual comparison.
"""
...
def render(df, X, y, w, claim_count, df_model, results):
    ...
    *This is the foundation of any insurance pricing structure, before loading for expenses, profit margin, and reinsurance.*
    ...
    pp_data["Severity"] = np.where(
        pp_data["Claims"] > 0, pp_data["TotalAmount"] / pp_data["Claims"], 0
    )
    ...
    fig_decomp.add_trace(go.Bar(
        ...
        text=pp_data["Severity"].round(0).astype(int).astype(str) + "\u20ac",
    ...
    fig_decomp.add_trace(go.Bar(
        ...
        text=pp_data["PurePremium"].round(0).astype(int).astype(str) + "\u20ac",
        textposition="outside",
    ), row=1, col=3)
    ...
    **Deep dive into claim severity distribution** — understanding the tail risk, extreme values, and distribution characteristics.
    ...
    if severity_analysis and len(sev_train) > 0:
        ...
        st.caption(
            ...
            "**TVaR (Tail Value at Risk):** Expected loss beyond VaR threshold"
        )
        ...
        st.info(
            f"💡 **Insight:** {extreme_pct:.1f}% of claims (\u226595th percentile) represent "
            f"{extreme_pct_of_total:.1f}% of total claim costs. "
            f"This highlights the importance of tail risk management."
        )
    else:
