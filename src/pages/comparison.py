...
Model benchmarking, lift curves, double lift analysis, SHAP interpretability.
"""
...
def render(df, X, y, w, claim_count, df_model, results):
    ...
    *Benchmarking the traditional actuarial approach (Poisson GLM) against a modern ML model (XGBoost).
    ...
    with m_col2:
        st.metric(
            "XGBoost Gini", f"{results['xgb_metrics_test']['Gini']:.4f}",
            delta=f"{results['xgb_metrics_test']['Gini'] - results['glm_metrics_test']['Gini']:.4f}",
        )
        st.metric(
            "XGBoost RMSE", f"{results['xgb_metrics_test']['RMSE']:.5f}",
    ...
    with m_col3:
        ...
        if xgb_gini > glm_gini:
            st.markdown(
                f"XGBoost achieves a **{((xgb_gini - glm_gini)/glm_gini)*100:.1f}% higher Gini**, "
    ...
    fig_lift = make_subplots(
        rows=1, cols=2,
        subplot_titles=("GLM Lift Curve", "XGBoost Lift Curve"),
    )

    fig_lift.add_trace(go.Bar(
        x=glm_lift["decile_label"], y=glm_lift["avg_actual"],
        name="Actual", marker_color=COLORS["primary"],
    ), row=1, col=1)
    fig_lift.add_trace(go.Scatter(
        x=glm_lift["decile_label"], y=glm_lift["avg_predicted"],
        name="GLM Predicted", mode="lines+markers",
        line=dict(color=COLORS["accent"], width=3),
    ), row=1, col=1)

    fig_lift.add_trace(go.Bar(
        x=xgb_lift["decile_label"], y=xgb_lift["avg_actual"],
        name="Actual", marker_color=COLORS["primary"], showlegend=False,
    ), row=1, col=2)
    fig_lift.add_trace(go.Scatter(
        x=xgb_lift["decile_label"], y=xgb_lift["avg_predicted"],
    ...
    fig_dl.add_trace(go.Bar(
        x=dl_agg["label"], y=dl_agg["avg_actual"],
        name="Actual", marker_color=COLORS["primary"],
    ))
    fig_dl.add_trace(go.Scatter(
        x=dl_agg["label"], y=dl_agg["avg_glm"],
        name="GLM", mode="lines+markers",
        line=dict(color=COLORS["accent"], width=2),
    ))
    fig_dl.add_trace(go.Scatter(
        x=dl_agg["label"], y=dl_agg["avg_xgb"],
    ...
    **SHAP values** explain each feature's contribution to individual predictions —
    ...
    try:
        ...
        fig_shap = px.bar(
            mean_shap, x="Mean |SHAP|", y="Feature", orientation="h",
