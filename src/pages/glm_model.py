...
Poisson GLM for frequency modeling and Gamma GLM for severity,
relativities, premium simulator, diagnostics, pricing table export.
"""
...
def render(df, X, y, w, claim_count, df_model, results):
    ...
    st.markdown(
        "*Poisson GLM for frequency modeling and Gamma GLM for severity "
        "— the industry standards for motor insurance pricing.*"
    )

    glm = results["glm_results"]

    st.caption(
        f"📐 *Model trained on a stratified sample of {len(X):,.0f} policies "
        f"(all claims retained) from {len(df):,.0f} total policies.*"
    )
    ...
    if not area_coefs.empty:
        ...
        fig_area = px.bar(
            area_coefs, x="Area", y="Relativity",
    ...
    try:
        ...
        sim_freq = df["Frequency"].mean() if "Frequency" in df.columns else 0.07
    ...
    if relativity_to_avg > 1.2:
        ...
        st.info("ℹ️ This profile is within **±20%** of portfolio average — standard risk.")
        ...
        **Statistical tests** validate the quality and assumptions of the GLM model.
        ...
        with res_col1:
            ...
            if len(pearson_res) > 0:
                ...
                fig_pearson = px.histogram(
                    x=pearson_res_sample, nbins=50,
        ...
        with res_col2:
            ...
            if len(deviance_res) > 0:
                ...
                fig_dev = px.histogram(
                    x=deviance_res_sample, nbins=50,
    ...
    pricing_display.columns = [
        ...
        "Policies", "Exposure (PY)", "Pure Premium (\u20ac)",
        "Commercial Premium (\u20ac)", "Relativity",
    ]
