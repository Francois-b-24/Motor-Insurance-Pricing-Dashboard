...
if page == "📊 Portfolio Overview":
    ...
    *Simulated monthly view of claim frequency evolution across the observation period (2003–2005).
    ...
    # Simulate monthly periods from the data (dataset has no date column)
    np.random.seed(123)
    n_policies = len(df)
    ...
    # Assign each policy a random inception month, weighted by exposure
    ...
    with trend_tab1:
        ...
        # Add linear trend line on frequency
        ...
        st.info(f"📊 **Trend:** The claim frequency shows a slight **{trend_direction}** trend "
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    **competitive positioning**, and **price elasticity** to build a market-ready tariff.*
    ...
    with exec_col2:
        target_lr = 0.65  # industry standard target loss ratio
    ...
    *These figures establish the **technical price floor**. The commercial premium must cover risk, expenses,
    ...
    # Loss Ratio at commercial premium
    ...
    **Price elasticity** measures how demand (policy count) responds to premium changes.
    ...
    with el_col1:
        ...
        base_elasticity = st.slider("Base Price Elasticity (ε)", -3.0, -0.1, -0.8, step=0.1,
    ...
    with el_col2:
        ...
        fig_elast.add_trace(
            go.Scatter(x=prices, y=demand, name="Demand (policies)",
                       line=dict(color=COLORS["secondary"], width=2.5),
                       fill="tozeroy", fillcolor="rgba(61,123,199,0.1)"),
            secondary_y=False
        )
        fig_elast.add_trace(
            go.Scatter(x=prices, y=profit, name="Profit (€)",
                       line=dict(color=COLORS["success"], width=2.5)),
            secondary_y=True
        )
        ...
        # Mark optimal point
        fig_elast.add_trace(
            go.Scatter(x=[optimal_price], y=[optimal_demand],
                       name=f"Optimal: €{optimal_price:.2f}",
                       mode="markers", marker=dict(size=14, color=COLORS["accent"],
                                                    symbol="star")),
            secondary_y=False
        )
    ...
    # Build segment-level pricing table
    ...
    # Simulate varying elasticities per segment
    np.random.seed(42)
    n_segs = len(seg_pp)
    ...
    # Visualization: Commercial Premium vs Elasticity
    fig_seg_opt = make_subplots(specs=[[{"secondary_y": True}]])
    fig_seg_opt.add_trace(
        go.Bar(x=seg_pp[seg_elast_var].astype(str), y=seg_pp["CommercialPremium"],
               name="Commercial Premium (€)", marker_color=COLORS["secondary"]),
        secondary_y=False
    )
    fig_seg_opt.add_trace(
        go.Scatter(x=seg_pp[seg_elast_var].astype(str), y=seg_pp["Elasticity"],
    ...
    *Simulated competitive landscape analysis. In production, this would be fed by market data,
    ...
    np.random.seed(42)
    competitors = ["Our Premium", "Competitor A\n(Traditional)", "Competitor B\n(Digital)",
                   "Competitor C\n(OEM Program)", "Competitor D\n(Bancassurance)"]
    comp_premiums = [commercial_premium,
    ...
    *Reliable pricing requires reliable data. This section monitors key data quality indicators —
    ...
    with dq_col1:
        ...
        completeness = {}
        key_fields = ["Exposure", "ClaimNb", "DrivAge", "VehAge", "VehPower",
        ...
        dq_df = pd.DataFrame({
            ...
            "Status": ["✅" if v >= 99 else ("⚠️" if v >= 95 else "❌") for v in completeness.values()]
        })
    ...
    with dq_col2:
        ...
        outliers = {}
        outlier_fields = {"BonusMalus": (50, 230), "Exposure": (0.01, 1.0),
